...
def create_stage_compat_router(
    *,
    ...
    async def get_production_state():
        if hub is not None and hasattr(hub, "get_production_state"):
            return {"status": "ok", "available": True, "state": hub.get_production_state()}
        return _unavailable("production_state")

    @router.post("/api/state/production")
    async def update_production_state(request: Request):
        if hub is None or not hasattr(hub, "update_production_state"):
            return _unavailable("production_state")
        patch = await _json_dict(request)
        updated = hub.update_production_state(patch)
        if hasattr(hub, "broadcast_system_event"):
    ...
    async def get_switcher_state():
        ...
        program = cameras.get_program_source() if hasattr(cameras, "get_program_source") else None
        preview = cameras.get_preview_source() if hasattr(cameras, "get_preview_source") else None
        return {
            "status": "ok",
            "available": True,
            "switcher": {
    ...
    async def set_switcher_state(request: Request):
        ...
        if program and hasattr(cameras, "set_program_source"):
            changed["program"] = program
            cameras.set_program_source(program)
        if preview and hasattr(cameras, "set_preview_source"):
            changed["preview"] = preview
            cameras.set_preview_source(preview)

        if hub is not None and hasattr(hub, "broadcast_system_event"):
            await hub.broadcast_system_event({"type": "switcher_state", "payload": changed})
